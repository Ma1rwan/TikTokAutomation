...
for video in videos:
    ...
    contentVideo_resized = contentVideo.resize(newsize=(tiktok_resolution[0], tiktok_resolution[1] // 2))
    videoBG_resized = videoBG.resize(newsize=(tiktok_resolution[0], tiktok_resolution[1] // 2))
    videoBG_resized = videoBG_resized.set_position(('center', 'bottom')).set_duration(contentVideo_resized.duration)
    contentVideo_resized = contentVideo_resized.set_position(('center', 'top')).set_duration(contentVideo_resized.duration)
    ...
    while part <= parts:
        ...
        final_part_clip = CompositeVideoClip([final_clip.subclip(clipStart, clipEnd), partClip, followVid], size=tiktok_resolution)
        final_part_clip.write_videofile(rf'D:\videos\stories\outputStories\{video[:-4]}\{video[:-4]}_part{part}.mp4',
